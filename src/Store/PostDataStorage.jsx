...
const DEFAULT_VALUE = JSON.parse(localStorage.getItem("allposts") || "[]");
...
const PostListProvider = ({ children }) => {
  ...
  useEffect(() => {
    localStorage.setItem("allposts", JSON.stringify(userPost));
  });

  const deletePost = (id) => {
    const postDeleted = userPost.filter((post) => post.id !== id);
    setUserPost(postDeleted);
    setPostFilter(postDeleted);
  };
